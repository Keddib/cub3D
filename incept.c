...
/*   Updated: 2020/11/09 05:53:47 by keddib           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	next_hincept(float n_x, float n_y, t_all *all, int i)
{
	while (n_x >= 0 && n_x <= (TILE_SIZE * all->win.rows) &&
			n_y >= 0 && n_y <= (TILE_SIZE * all->win.cols))
	{
		all->incept.check_x = n_x;
		all->incept.check_y = n_y + (all->ray[i].facingup ? -1 : 0);
...
void	next_vincept(float x, float y, t_all *all, int i)
{
	while (x >= 0 && x <= (TILE_SIZE * all->win.rows) &&
			y >= 0 && y <= (TILE_SIZE * all->win.cols))
	{
		all->incept.check_x = x + (all->ray[i].facingleft ? -1 : 0);
...
void	get_vertical(t_all *all, int i)
{
	...
	all->incept.xintercept = floor(all->fpp.x / TILE_SIZE) *
		TILE_SIZE;
	all->incept.xintercept += all->ray[i].facingright ?
		TILE_SIZE : 0;
	all->incept.yintercept = all->fpp.y +
		(all->incept.xintercept - all->fpp.x) *
		tan(all->ray[i].ray_angle);
	all->incept.xstep = TILE_SIZE;
	all->incept.xstep *= all->ray[i].facingleft ? -1 : 1;
	all->incept.ystep = TILE_SIZE * tan(all->ray[i].ray_angle);
	all->incept.ystep *=
		(all->ray[i].facingup && all->incept.ystep > 0) ? -1 : 1;
	all->incept.ystep *=
		(all->ray[i].facingdown && all->incept.ystep < 0) ? -1 : 1;
	next_x = all->incept.xintercept;
	next_y = all->incept.yintercept;
	next_vincept(next_x, next_y, all, i);
}

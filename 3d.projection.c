...
/*   Updated: 2020/10/26 01:22:29 by keddib           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_walls(t_ray ray, int i, int ws_height, t_texture *tex)
{
    ...
    {
        offy = (start - (g_window.half_win - (ws_height / 2))) * ((float)TILE_SIZE / ws_height);
...
void render_3d_projection(t_ray ray, t_texture *tex, int i)
{
    ...
    ray.wall_top = g_window.half_win - (wall_stripe_hieght / 2);
    if (ray.wall_top < 0)
        ray.wall_top = 0;
    ray.wall_bottom = g_window.half_win + (wall_stripe_hieght / 2);

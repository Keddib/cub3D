...
/*   Updated: 2020/10/20 13:45:57 by keddib           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_walls(t_ray *ray, int i, int ws_height, t_texture *tex)
{
	...
	while (start < ray[i].wall_bottom)
	{
		...
		color = tex[3].data[(TILE_SIZE * offy) + offx];
...
void render_3d_projection(t_ray *rays, t_texture *tex)
{
	int i;
	...
	while (i < window.width)
	{
		...
		render_walls(rays, i++, wall_stripe_hieght, tex);
	}
}
